...
import numpy as np
# Warnings kaldırma (daha temiz çıktı için)
import warnings
warnings.filterwarnings('ignore')
from src.utils.logger import get_logger
logger = get_logger()

# Pandas görüntüleme ayarları - veri çıktılarını daha okunabilir hale getirir
pd.set_option('display.max_columns', None)  # Tüm sütunları göster, kesme yapma
pd.set_option('display.max_rows', None)     # Tüm satırları göster, kesme yapma
pd.set_option('display.float_format', lambda x: '%.2f' % x)  # Float sayıları 2 ondalık basamakla göster
pd.set_option('display.width', 1000)       # Çıktı genişliğini 1000 karaktere ayarla
RAW_DATA_PATH="data/raw/train.csv"
PROCESSED_DATA_PATH="data/processed/"
# Matplotlib ayarları
plt.style.use('seaborn-v0_8')  # Daha güzel görselleştirme stili
plt.rcParams['figure.figsize'] = (12, 8)  # Varsayılan grafik boyutu

logger.info(f"Veri yükleniyor: {RAW_DATA_PATH}")

    
# CSV dosyasını okuma - flo müşteri verilerini içeren dosyayı yükler
df_ = pd.read_csv(RAW_DATA_PATH)  # Dosya yolu düzeltildi
# Orijinal veriyi korumak için kopya oluşturma - veri manipülasyonu sırasında orijinal veriyi kaybetmemek için
df = df_.copy()
